...
export async function recordUsage(model, usage) {
  ...
  ui.notifications.info(
    cost === null
      ? game.i18n.format('MCC.Usage.TokensOnly', { input, output })
      : game.i18n.format('MCC.Usage.Reported', {
          input,
          output,
          cost: cost.toFixed(4),
...
export async function requestStructured(options) {
  ...
  for (let attempt = 0; attempt <= RETRY_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (error.name === 'AbortError' || !error.mccRetryable) throw error;
      lastError = error;
      if (attempt < RETRY_ATTEMPTS) {
        ui.notifications.warn(
          game.i18n.format('MCC.Errors.Retrying', { message: error.message }),
        );
      }
    }
  }
...
const retryable = (error) => Object.assign(error, { mccRetryable: true });
...
async function attemptStructured({ schemaName, schema, system, user, signal }) {
  ...
  if (!apiKey) throw new Error(game.i18n.localize('MCC.Errors.NoApiKey'));
  ...
  try {
    ...
    if (timedOut) {
      throw retryable(new Error(game.i18n.format('MCC.Errors.Timeout', { seconds })));
    }
    if (error.name === 'AbortError') throw error;
    throw retryable(new Error(game.i18n.format('MCC.Errors.Network', { message: error.message })));
  } finally {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      game.i18n.format('MCC.Errors.Api', { status: response.status, message: detail }),
    );
  ...
  if (message?.refusal) {
    throw new Error(game.i18n.format('MCC.Errors.Refusal', { message: message.refusal }));
  }
  if (!message?.content) {
    throw retryable(new Error(game.i18n.localize('MCC.Errors.EmptyResponse')));
  }
  ...
  try {
    ...
    throw retryable(new Error(game.i18n.format('MCC.Errors.BadJson', { message: error.message })));
  }
