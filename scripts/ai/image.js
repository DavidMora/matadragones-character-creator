...
function apiKey() {
  ...
  if (!key) throw new Error(game.i18n.localize('MCC.Errors.NoApiKey'));
  return key;
}
...
export async function fetchReference(src) {
  ...
  if (!blob.type.startsWith('image/')) {
    throw new Error(game.i18n.localize('MCC.Errors.NotAnImage'));
  }
...
async function rasterise(blob) {
  ...
  try {
    const image = await new Promise((resolve, reject) => {
      ...
      element.onerror = () => reject(new Error(game.i18n.localize('MCC.Errors.NotAnImage')));
      element.src = url;
    });
...
export async function generateImage({ prompt, references = [], size, quality, signal }) {
  ...
  try {
    ...
    throw new Error(game.i18n.format('MCC.Errors.Network', { message: error.message }));
  }
  ...
  if (!first?.b64_json) throw new Error(game.i18n.localize('MCC.Errors.NoImageReturned'));
